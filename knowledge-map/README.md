...
这是 T01/T02/T03 的本地只读实现：从维护中的 AI 工程 Wiki 页面生成版本化 `MapSnapshot`，并通过 B · List-first Navigator 导航。快照同时包含显式页面链接图和本地确定性的语义探索图；查看器在同一个不可变快照上按 seed、层、depth、关系预算和 filters 请求有边界的子图。
